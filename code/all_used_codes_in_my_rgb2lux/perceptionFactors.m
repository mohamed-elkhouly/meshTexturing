function [Fij] = perceptionFactors(cadModel, rays, method, ldc, Fij, positions, head_orientation)
    ...
    %args
    if nargin < 5
        error('Position of detected persons are needed!!!!');
    else
        patches = size(positions,1);
%         if size(patches,1)<size(patches,2)
%             patches = patches';
%         end
    end
    if nargin < 4 || isempty(Fij)
        % initialize the view/form factors matrix
        Fij = zeros(size(positions,1),size_n);
    end
%     if nargin < 4 
%         % if beckers is chosen the specify technique the rays would be
%         % arranged, i.e. random = 1 or deterministic = 2
...
%}
    t = opcodemesh(v',f');
    for i = patches;
        r = vrrotvec(head_orientation(i,:),[0 0 1]); % Calculate rotation between two vectors, first vector is the normal of the facet and second vector is normal of the isocel which is always [0 0 1] by default
...
%         plane = createPlane(positions(i,:), head_orientation(i,:));
%         
...
%         [tt,uu,vv,idxx,xnodee]=raysurf(repmat(positions(i,:), size(draysR,1), 1), draysR , v, f(filteredFaces,:));
        [hit,tt,idxx,bary,xnodee] = t.intersect(repmat(positions(i,:), size(draysR,1), 1)',draysR');
        xnodee = xnodee';
...
%}
            ...
            ind22 = ~isFacing(positions(i, :), head_orientation(i, :), centroids(intersections, :), normals(intersections, :));
        ...
        if ~isempty(intersections) && ~isempty(ldc)
            rayPnts = draysR + repmat(positions(i,:), rays, 1); % create points on the rays and their directions
