function [F] = viewFactors4OPCODE_sparse_version_0_0(cadModel, rays, method, ldc, Fij, ldcPatches)
...
%     current_pose_spec_faces=cadModel.specularities_in_frame(frame);
...
%         current_spec_face_normal=normals(current_pose_spec_faces(ray_index),:);
...
%         [hit,tt,idxx,bary,xnodee] = t.intersect(centroids(current_pose_spec_faces(ray_index),:)',mirrored_ray');
%         intersections=idxx;
% %         intersections = unique(idxx); % find which faces are hit from the rays
%         intersections(intersections==0)=[];
%         intersections(isnan(intersections) | isinf(intersections)) = []; % remove indices of non intersected rays
...
%     cam_val_v=[F_val_V,F_val_V];
    ...
    F=sparse(F_row_V,F_col_V,F_val_V,size_n+size(region_specular_frames,1),size_n+size(region_specular_frames,1));
