function [F] = viewFactors4OPCODE_sparse2(cadModel, rays, method, ldc, F, ldcPatches)
%     global iteration;
%     iteration=iteration+1;
%     if iteration==1
%         % data_vectors(:,1)>> i-index    data_vectors(:,j)>> j-index
%     data_vectors=zeros(ceil(size_n*size_n*0.01),3);
%     end
    ...
    if nargin < 4
        ...
        F = spalloc(size_n,size_n,ceil(size_n*size_n*0.01));
...
%                 tmp = find(idxx == intersections(ii));
                amount_of_intersections(ii) = sum(descriptor.candelasNorm(idxx == intersections(ii)));
            end
        else
...
%         fprintf(' (iteration %i) : %i : %i \n',i, length(intersections), length(amount_of_intersections));
        F(i,intersections) = amount_of_intersections;
%         if ~isempty(valid_rays) %|| valid_rays ~= 0
%             Fij(i,:) = Fij(i,:) / valid_rays;
%         else
            F(i,:) = F(i,:) / rays; % normalize view factor according to the total rays thrown each time
%         end
