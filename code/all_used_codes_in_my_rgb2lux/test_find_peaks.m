...
new_y_upper=uint8(zeros(size(y)));
new_y_lower=uint8(zeros(size(y)));
new_y_upper(y>=median_value)=y(y>=median_value);
new_y_upper=new_y_upper(:);
new_y_upper(new_y_upper==0)=[];
upper_thresh_values = multithresh(new_y_upper,20);

new_y_lower(y<median_value)=y(y<median_value);
new_y_lower=new_y_lower(:);
% new_y_lower(new_y_lower==0)=[];
lower_thresh_values = multithresh(new_y_lower,20);
