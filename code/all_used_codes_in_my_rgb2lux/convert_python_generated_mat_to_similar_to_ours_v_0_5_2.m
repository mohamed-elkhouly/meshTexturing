...
for k=1:length(mesh.campos)
    ...
    for i=1:Num_holes
        ...
        if(~isempty(boundary_faces))
            ...
            %%
            distances_to_cam=sort(vecnorm((mesh.v(boundary_vertices,:)-mesh.campos(k,:))'));
...
figure;
plot_CAD(mesh.f, mesh.v, '',[zeros([size(mesh.f_c,1) 1]); 200*ones([(size(mesh.f,1)-size(mesh.f_c,1)),1])]);
savefig(['region',num2str(region_number) ,scene_name,'_hole_filled_0.0003_1.01_1st_iteration.fig'])
