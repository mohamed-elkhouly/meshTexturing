...
if (smooth)
    ...
    try
        load(['smoothed_region',num2str(region_number),'_',scene_name,'.mat']);
    catch
        [mesh.v,mesh.f]=smooth_mesh(region_number,mesh.v,mesh.f,mesh.normals,mesh.centroids);
        save(['smoothed_region',num2str(region_number),'_',scene_name,'.mat'],'mesh');
    end
    % figure, plot_CAD(mesh.f, mesh.v, '');
end
