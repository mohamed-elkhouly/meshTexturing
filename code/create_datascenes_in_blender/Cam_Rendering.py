...
def store_light_information():                
    ############################################################################                
    # storing light information 
    data={}
    data['lights'] = []
    path_lights=os.path.join(directory,"lights_info")
    print(path_lights)
    try:
        os.mkdir(path_lights)
    except:
        print ("Creation of the directory %s failed" % path_lights)
        if os.path.isdir(path_lights):
            
            for root, dirs, files in os.walk(path_lights):
                for file in files:
    ...
    for obj in bpy.data.objects:
         ...
         #print(obj.type)
         # I wanted to make this condition for those lights configured for VI-Suite which has another different imported ies object corresponding to it.
         # in other words, when we want to use IES profiles, we import ies profile, but it is imported as object not as lamp, this profile work in rendering, but will not work for vi-suite, to have light analysis in VI-Suite, we add a lamp, and make its default blender Nodes>Strength=0, then in VI-Suite, we import the ies profile for it. also do not forget that it should have the same position and orientation of ies profile.
         
         if(obj.type== "LAMP" and obj.hide==False):
            if(obj.data.type=='POINT' or obj.data.type=='SUN' or obj.data.type=='SPOT' or obj.data.type=='HEMI'):
                color=bpy.data.lamps[obj.name].node_tree.nodes['Emission'].inputs[0].default_value
                strength=bpy.data.lamps[obj.name].node_tree.nodes['Emission'].inputs[1].default_value
                #print(color[0], color[1], color[2], color[3])
                #print(strength)
            '''    
            try:
                lamp_size=obj.shadow_soft_size
            except:
                try:
                    lamp_size=obj.size
                except:
            ...
            print("lamp size is ", lamp_size)    
            '''
            
            VI_ies=obj.ies_name
            VI_color=obj.ies_rgb
            VI_strength=obj.ies_strength            
            location= np.array([obj.location.x, obj.location.y, obj.location.z])
            orientation= np.array([obj.rotation_euler.x, obj.rotation_euler.y, obj.rotation_euler.z])
            #dimention=obj.dimensions.z
            #print("VI_ies is ",VI_ies)
            #print("VI_color is ", VI_color[0], VI_color[1], VI_color[2])
            #print("VI_strength is ", VI_strength)
            #print("location is ",location)
            #print(dimention)
            #print("orientation is ",orientation*180/3.14)        
         #==============================
         # this next situation here will work for any kind of blender object which has emission shader node
         ...
         if(len(bpy.context.object.material_slots)>0):         
             for mat_slot, m in enumerate(bpy.context.object.material_slots):
                    mat=m.material       
                    ...
                    if mat.use_nodes == True:
                        ...
                        # Iterate through all the nodes in the node tree and test each one to see if it's an emission shader:
                        for node in nodes:
                            node.select=False
                            if(node.bl_idname=='ShaderNodeEmission'):
                                is_light=True
                                print (obj.name , ' has emission node ')
                                node.select=True
                                # this try for emission which is applied to mesh, in this case, the object will have faces, otherwise, it will be a lamp
                                try:#print('it has mesh')
                                ...
                                lightcenter=np.array([0,0,0])
                                num_faces=0
                                found_light_mesh=False
                                for face in bm.faces:
                                    face.select=True
                                    if (obj.material_slots[face.material_index].name == mat.name):
                                        found_light_mesh=True
                                        f_cent=obj.matrix_world * bpy.data.objects[obj.name].data.polygons[face.index].center
                                        #f_norm=obj.matrix_world * bpy.data.objects[obj.name].data.polygons[face.index].normal                                        
                                        lightcenter=lightcenter+np.array(f_cent)
                                        num_faces=num_faces+1
                                    face.select=False
                                if(found_light_mesh):
                                    light_type='mesh'
                                    color=np.array(node.inputs[0].default_value)
                                    strength=np.array(node.inputs[1].default_value)
                                    location=np.array(lightcenter/num_faces)
                                    orientation=np.empty([])
                                    size=np.empty([])
                                    #print(location)
                                bpy.ops.object.mode_set(mode='OBJECT')
        # here it mean that the object does not has material, we have to check if it is light or no, then take its position, and other info.
         else:               
             if(obj.type== "LAMP" and obj.hide==False):
                is_light=True 
                light_type=bpy.data.lamps[obj.name].type
                try:# try to get color or strength properties for this type of lamp
                    color=np.array(bpy.data.lamps[obj.name].node_tree.nodes['Emission'].inputs[0].default_value)
                    strength=np.array(bpy.data.lamps[obj.name].node_tree.nodes['Emission'].inputs[1].default_value)
                except:# mean there are no color or strength properties for this type of lamp
                    color=np.empty([])
                    strength=np.empty([])        
                location= np.array([obj.location.x, obj.location.y, obj.location.z])
                orientation= np.array([obj.rotation_euler.x, obj.rotation_euler.y, obj.rotation_euler.z])
                for obj_lamp in bpy.data.lamps:
                    if(obj_lamp.name==obj.name):
                        print(obj_lamp.name)
                        size=np.empty([])
                        if hasattr(obj_lamp, 'size'):
                            size=np.array(getattr(obj_lamp, 'size'))
                        if hasattr(obj, 'shadow_soft_size'):
                            size=np.array(getattr(obj_lamp, 'shadow_soft_size'))
         if(is_light):
             print(obj.name)
             print(location)
             print(orientation)            
             ...
             data['lights'].append({
                ...
                'IndexOB':obj.pass_index                                     
             })                  

    with open((path_lights+"/lights.txt"), 'w') as outfile:
...
try:
    ...
    print('can not convert to object mode')
