function [hitted_faces_2,faces_places_in_image,back_idxx,index_f,index_in_hitted_faces_1,index_in_all_used_group_faces_1]=get_hitted_faces_in_mesh(i,mesh)
%     if(~using_different_mesh)
%         file_path=[mesh.data_path,'/frames_faces_mapping/','frame-',sprintf('%06d',i),'.color.png'];
...
%     else
        idxx=mesh.frame_face_mapping(i,:,:);
        idxx=reshape(idxx,mesh.frame_height,mesh.frame_width);
...
%     idxx(idxx>end_of_faces_indexing)=[];
    ...
    all_used_groups_faces=find(mesh.all_visible_faces);
  
    [~,index_in_all_used_group_faces_1,index_in_hitted_faces_1]=intersect(all_used_groups_faces,hitted_faces);
