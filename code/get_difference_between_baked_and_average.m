function get_difference_between_baked_and_average(mesh)

average_face_colors_mat=[mesh.data_path,mesh.scene_name,'_average_colors_per_face.mat'];
baked_face_colors_mat=[mesh.data_path,mesh.scene_name,'_baked_colors_per_face.mat'];
mkdir([mesh.data_path,'baked_images']);
mkdir([mesh.data_path,'averaged_images']);
mkdir([mesh.data_path,'difference_images']);
load(average_face_colors_mat)
load(baked_face_colors_mat)

final_average_colors=final_average_colors;
...
for i =1:size(mesh.frame_face_mapping,1)
    ...
    imwrite(uint8(average_image),[mesh.data_path,'averaged_images','/',num2str(i),'.jpg']);
    ...
    colormap jet
    colorbar
    saveas(gcf,[mesh.data_path,'difference_images','/',num2str(i),'.jpg'])
end
