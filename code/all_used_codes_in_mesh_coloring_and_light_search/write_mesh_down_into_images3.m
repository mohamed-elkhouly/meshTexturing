function write_mesh_down_into_images3(mesh,region_frames,final_faces,final_vertices,final_faces_colors,scene_name,region_number,flag)
...
mkdir(['mesh_coloring/',scene_name,'/',num2str(region_number),'/averaging']);
...
for kk=1:size(region_frames,1)
     i=region_frames(kk);
    faces_image_indexes=1:image_height*image_width;% this is to store for each pixel the nearest face to it and its distance.
     hitted_mesh_pixels=[];
    [hitted_mesh_pixels(2,:), hitted_mesh_pixels(1,:)]=ind2sub([image_height image_width],faces_image_indexes);
...
%    image_of_faces=reshape(idxx,image_height,image_width);
red=zeros(image_height,image_width);
green=zeros(image_height,image_width);
blue=zeros(image_height,image_width);
   red(idxx~=0)=final_faces_colors(idxx(idxx~=0),1);
   green(idxx~=0)=final_faces_colors(idxx(idxx~=0),2);
   blue(idxx~=0)=final_faces_colors(idxx(idxx~=0),3);
    new_image(:,:,1)=red;
    new_image(:,:,2)=green;
    new_image(:,:,3)=blue;
%     new_image=reshape(new_image,[1024,1280,3]);
     new_image=uint8(new_image);
    imwrite(new_image,['mesh_coloring/',scene_name,'/',num2str(region_number),'/averaging/frame-',sprintf('%06d',i),'.color.png']);
