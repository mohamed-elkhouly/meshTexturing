function [textures_images_and_faces]=create_A_omega_F_matrices2(vertices_on_border_info,mesh,border_vertices_coord,G_vertices,textures_images_and_faces)
...
for i=1:size(vertices_on_border_info,1)
    ...
    for j=1:size(vertices_on_border_info,1)
        % to skip overlapping with self and work only with the upper
        % triangle 
        if(i>=j)
            continue;
        end
