function r = vrrotvec_multiple(a, b, options)
...
bn = vrnormalize_multiple(b, epsilon);
for i=1:size(bn,1)
    cross_prod(i,:)=cross(an, bn(i,:));
    dot_prod(i,:)=dot(an, bn(i,:));
end
axb = vrnormalize(cross_prod, epsilon);
ac = acos(dot_prod);
