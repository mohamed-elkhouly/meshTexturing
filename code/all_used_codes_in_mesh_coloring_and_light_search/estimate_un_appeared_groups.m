function [new_group,grand_group]=estimate_un_appeared_groups(group,all_faces_in_group,mesh,group_index,grand_group)
...
new_group=group;
if(~isempty(group.should_be_estimated))
    groups_to_be_estimated=group.should_be_estimated;
    ...
    for frame_index=1:size(intersected_frames,1)
        ...
        for oth_f_index=1:length(other_required_frames)
            ...
            if(length(C_f_indsa)<100)
                ...
                if(length(C_f_indsa)/length(all_faces_in_group)<0.3)
                    continue;
                end
            end
            ...
            if(~isempty(division_vals)|~isempty(division_vals_a)|~isempty(division_vals_b))
                ...
                group_to_be_estimated_from_this_frame=groups_to_be_estimated(groups_to_be_estimated(:,1)==other_frame,2);
                for t_b_e_gr_index=1:length(group_to_be_estimated_from_this_frame)
                    gr_index=group_to_be_estimated_from_this_frame(t_b_e_gr_index);
                    
                    est_gr_faces=grand_group(gr_index).frame(other_frame).faces;
                    
                    est_gr_values=grand_group(gr_index).frame(other_frame).values;
                    est_gr_values_a=grand_group(gr_index).frame(other_frame).values_a;
                    est_gr_values_b=grand_group(gr_index).frame(other_frame).values_b;
                    
                    est_gr_values=average_div_vals*est_gr_values;
                    est_gr_values_a=average_div_vals_a*est_gr_values_a;
                    est_gr_values_b=average_div_vals_b*est_gr_values_b;
                    
                    try
                        already_existed_faces=grand_group(gr_index).frame(current_frame).faces;
                    catch
                        grand_group(gr_index).frame(current_frame).faces=[];
                        grand_group(gr_index).frame(current_frame).values=[];
                        grand_group(gr_index).frame(current_frame).values_a=[];
                        grand_group(gr_index).frame(current_frame).values_b=[];
                        already_existed_faces=[];
                    end
                    try
                        other_est_faces=grand_group(gr_index).frame(current_frame).est_faces;
                        other_est_values=grand_group(gr_index).frame(current_frame).est_values;
                        other_est_values_a=grand_group(gr_index).frame(current_frame).est_values_a;
                        other_est_values_b=grand_group(gr_index).frame(current_frame).est_values_b;
                    catch
                        grand_group(gr_index).frame(current_frame).est_faces=[];
                        grand_group(gr_index).frame(current_frame).est_values=[];
                        grand_group(gr_index).frame(current_frame).est_values_a=[];
                        grand_group(gr_index).frame(current_frame).est_values_b=[];
                        other_est_faces=[];
                        other_est_values=[];
                        other_est_values_a=[];
                        other_est_values_b=[];
                    end
                    % check if there are connection with original frame faces?
                    [~,already_exd]= intersect(est_gr_faces,already_existed_faces);
                    est_gr_faces(already_exd)=[];
                    est_gr_values(already_exd)=[];
                    est_gr_values_a(already_exd)=[];
                    est_gr_values_b(already_exd)=[];
                    
                    % check if there are connection with other estimated faces for the same frame?
                    [~,already_exd,oth_est_index]= intersect(est_gr_faces,other_est_faces);
                    other_est_faces(oth_est_index)=[];
                    est_gr_values(already_exd)=mean([est_gr_values(already_exd),other_est_values(oth_est_index)],2);
                    est_gr_values_a(already_exd)=mean([est_gr_values_a(already_exd),other_est_values_a(oth_est_index)],2);
                    est_gr_values_b(already_exd)=mean([est_gr_values_b(already_exd),other_est_values_b(oth_est_index)],2);
                     other_est_values(oth_est_index)=[];
                        other_est_values_a(oth_est_index)=[];
                        other_est_values_b(oth_est_index)=[];
                    grand_group(gr_index).frame(current_frame).est_faces=[other_est_faces;est_gr_faces];
                    grand_group(gr_index).frame(current_frame).est_values=[other_est_values;est_gr_values];
                    grand_group(gr_index).frame(current_frame).est_values_a=[other_est_values_a;est_gr_values_a];
                    grand_group(gr_index).frame(current_frame).est_values_b=[other_est_values_b;est_gr_values_b];
                    grand_group(gr_index).frames_index=unique([grand_group(gr_index).frames_index;current_frame]);
                end
...
end% the end of the if condition for checking the should_be_estimated to not be empty
d=[];