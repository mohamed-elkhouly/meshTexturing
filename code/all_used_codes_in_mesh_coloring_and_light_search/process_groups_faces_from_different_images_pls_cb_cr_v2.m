function process_groups_faces_from_different_images_pls_cb_cr_v2(mesh,faces_correspondences,region_faces,regions_edge_faces,all_used_groups_faces,region_frames,scene_name,folder_path,region_number)
...
%%
% sort frames numbers and nuber of groups faces in it. based on
% frame_number
% num_faces_in_frames=[frame, group_index, num_faces_existed_from_group, num_all_faces_in_group, ratio_between last two columns]
num_faces_in_frames=[num_faces_in_frames,zeros(size(num_faces_in_frames(:,1)))];
group_faces_cells=region_faces(num_faces_in_frames(:,2));
for i=1:size(num_faces_in_frames,1)
    num_faces_in_frames(i,4)=length(cell2mat(group_faces_cells(i)));
end
num_faces_in_frames=[num_faces_in_frames,num_faces_in_frames(:,3)./num_faces_in_frames(:,4)];
sorting_column=num_faces_in_frames(:,1);
[sorted_column,ia]=sort(sorting_column);
num_faces_in_frames=num_faces_in_frames(ia,:);
for index=1:size(region_frames,1)
    current_frame=region_frames(index)+1;
    current_frame_correspondences(current_frame).groups_faces=num_faces_in_frames(sorted_column==current_frame,:);
    sorting_column=current_frame_correspondences(current_frame).groups_faces(:,3);
    [temp_sorted_column,ia]=sort(sorting_column,'descend');
    ratio_of_num_faces_to_tot_faces_in_group=current_frame_correspondences(current_frame).groups_faces(ia,5);
    ia(temp_sorted_column<50&ratio_of_num_faces_to_tot_faces_in_group<0.25)=[];
    current_frame_correspondences(current_frame).groups_faces=current_frame_correspondences(current_frame).groups_faces(ia,:);
    %     current_frame_correspondences(current_frame).groups_faces(:,[3,4,5])=[];% no need for them anymore
    %     next few lines is to assign the mession of estimating other groups to the first two groups of the list
    if(size(current_frame_correspondences(current_frame).groups_faces,1)>1)
        groups(current_frame_correspondences(current_frame).groups_faces(1,2)).should_be_estimated=[groups(current_frame_correspondences(current_frame).groups_faces(1,2)).should_be_estimated;current_frame_correspondences(current_frame).groups_faces(2:end,:)];
        
        groups(current_frame_correspondences(current_frame).groups_faces(2,2)).should_be_estimated=[groups(current_frame_correspondences(current_frame).groups_faces(2,2)).should_be_estimated;...
            current_frame_correspondences(current_frame).groups_faces(1,:)];
        if(size(current_frame_correspondences(current_frame).groups_faces,1)>2)
            groups(current_frame_correspondences(current_frame).groups_faces(2,2)).should_be_estimated=[groups(current_frame_correspondences(current_frame).groups_faces(2,2)).should_be_estimated;...
                current_frame_correspondences(current_frame).groups_faces(3:end,:)];
        end
    end
end
d=[];
...
% for group_index=1:length(region_faces)
%     
%     [~,groups1]=estimate_un_appeared_groups(groups(group_index),cell2mat(region_faces(group_index)),mesh,group_index,groups1);
%         groups1(group_index).frame(888).faces=[];
%     groups1(group_index).frame(888).values=[];
%     groups1(group_index).frame(888).values_a=[];
%     groups1(group_index).frame(888).values_b=[];
%     groups1(group_index).frame(888).est_faces=[];
%     groups1(group_index).frame(888).est_values=[];
%     groups1(group_index).frame(888).est_values_a=[];
%     groups1(group_index).frame(888).est_values_b=[];
% end
% toc
% 
% for group_index=1:length(region_faces)
%     for frame_index=1:length(groups1(group_index).frame)
%         groups1(group_index).frame(frame_index).faces=[groups1(group_index).frame(frame_index).faces;groups1(group_index).frame(frame_index).est_faces];
%         groups1(group_index).frame(frame_index).values=[groups1(group_index).frame(frame_index).values;groups1(group_index).frame(frame_index).est_values];
%         groups1(group_index).frame(frame_index).values_a=[groups1(group_index).frame(frame_index).values_a;groups1(group_index).frame(frame_index).est_values_a];
%         groups1(group_index).frame(frame_index).values_b=[groups1(group_index).frame(frame_index).values_b;groups1(group_index).frame(frame_index).est_values_b];
%     end
% end
% d=[];
% for show only
